...
public class InsumoController : Controller
{
    ...
    // GET: /Insumo (Página Segmentada de Insumos e Ferramentas em 2 Colunas)
    [HttpGet]
    public async Task<IActionResult> Index(string? busca)
    {
        var produtosInsumos = await _produtoRepository.ListarPorCategoriasAsync("insumo", "ferramenta");

        if (!string.IsNullOrWhiteSpace(busca))
        {
            var termo = busca.Trim().ToLower();
            produtosInsumos = produtosInsumos.Where(p => 
        ...
        {
            ProdutosDestaque = produtosInsumos,
